...
$redirect_uri = "https://node53.webte.fei.stuba.sk/z1/oauth2callback.php"; // Redirect URI for the OAuth2 callback. Must match the one in the Google Cloud Console.
...
if (isset($_GET['code'])) {
    ...
    // Skontrolovať, či užívateľ existuje v databáze
    $stmt = $db->prepare("SELECT id FROM users WHERE email = :email");
    $stmt->bindParam(":email", $_SESSION['email'], PDO::PARAM_STR);
    $stmt->execute();
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    
    $user_id = $user ? $user["id"] : null; // Ak existuje user, získaj ID, inak nastav na NULL
    $login_type = "google";
    
    // Uloženie loginu aj keď user_id je NULL
    $stmt = $db->prepare("
    ...
    $stmt->execute();
    $redirect_uri = 'https://node53.webte.fei.stuba.sk/z1/restricted.php'; // Redirect to the restricted page or dashboard.
